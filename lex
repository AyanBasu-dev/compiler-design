...
int compare(char *a, char *b)
{
	...
	while(*(a+ct)!='\0')
	{
		if(((int)*(b+ct)-(int)*(a+ct))!=0) {return 0;}
		ct++;
	}
	return 1;
}
int checkop(char ch1, char ch2)
{
	...
	*txt = ch1; *(txt+1) = ch2; *(txt+2) = '\0';
	char *op[] = {"++", "--", "==", ">=", ">>", "<=", "<<", "&&", "||", "!=", "**",	"//", "/*", "*/", "::"};
	while(i<15) 
	{
		if(compare(op[i], txt)) return 1;
		i++;
	}
	return 0;
}
...
{
	char *keyset[] = {"auto","break","case","char","const","continue","default","do","double","else","enum","extern","float","for","goto","if","int","long","register","return","printf", "scanf", "short","signed","sizeof","static","struct","switch","typedef","union","unsigned","void","volatile","while"};
	char *opset[] = {"+", "-", "*", "/", "%", "=", ">", "<", "&", "|", "!", "++", "--", "==", ">=",	">>", "<=", "<<", "&&", "||", "!="};
	int i, clss=1;
	if(*str=='#') {
	...
	switch(clss)
	{
		case 0: fprintf(ptr, "%s \t --Not Recognized\n", str);
		break;
		case 1: fprintf(ptr, "%s \t --Identifier\n", str);
		break;
		case 2: fprintf(ptr, "%s \t --Keyword\n", str);
		break;
		case 3: fprintf(ptr, "%s \t --Operator\n", str);
		break;
		case 4: fprintf(ptr, "%s \t --Number\n", str);
	}
}
void getwd()
{
